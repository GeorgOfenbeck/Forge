...
trait RewriteOpsExp extends RewriteOps with TableOpsExp {
  ...
  private def hashReduce[A:Manifest,K:Manifest,T:Manifest,R:Manifest](resultSelector: Exp[T] => Exp[R], keySelector: Exp[A] => Exp[K]): Option[(Exp[A]=>Exp[R], (Exp[R],Exp[R])=>Exp[R], (Exp[R],Exp[Int])=>Exp[R])] = {
    ...
    def rewriteReduce[N](value: Exp[Any]) = (value match {
      ...
      case Def(d@Table_Sum(_,_)) => (a:Exp[N],b:Exp[N]) => numeric_pl(a,b)(ntype(d.__imp0),mtype(d._mR),ctx)
      case Def(d@Table_Average(_,_)) => (a:Exp[N],b:Exp[N]) => numeric_pl(a,b)(ntype(d.__imp1),mtype(d._mR),ctx)
    ...
    def rewriteAverage[N](value: Exp[Any]) = (value match {
      case Def(d@Table_Average(_,_)) => (a:Exp[N],count:Exp[Int]) => fractional_div(a, count.asInstanceOf[Exp[N]])(mtype(d._mR),frtype(d.__imp0),mtype(d._mR),ctx,implicitly[Rep[N]=>Rep[N]])
