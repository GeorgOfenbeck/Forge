...
object QPSCDCompiler extends AutoOptiMLApplicationCompiler with QPSCD {
  val mBQP: Manifest[BQP] = manifest[BQP]
}
object QPSCDInterpreter extends AutoOptiMLApplicationInterpreter with QPSCD {
  val mBQP: Manifest[BQP] = manifest[BQP]
}

trait QPSCD extends AutoOptiMLApplication {
