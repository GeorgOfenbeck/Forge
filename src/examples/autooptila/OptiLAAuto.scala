...
object AutoOptiLADSLRunner extends ForgeApplicationRunner with AutoOptiLADSL

trait AutoOptiLADSL extends ForgeApplication
  with BasicMathOps with RandomOps with IOOps with ArithOps with StringableOps
  with DenseVectorOps with IndexVectorOps with DenseVectorViewOps with DenseMatrixOps
  with LinAlgOps with ManualOps { this: ppl.dsl.forge.core.ForgeOpsExp =>
  ...
  def specification() = {
    ...
    // TODO foreach should be the next method
    // compiler (Range) ("infix_foreach", List(tpePar("T")), (Range, MInt ==> MUnit) :: MUnit) implements composite ${ range_foreach(range_start($0), range_end($0), $1) }
