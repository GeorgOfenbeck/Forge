...
trait AutoOptiQLDSL extends ForgeApplication { this: ppl.dsl.forge.core.ForgeOpsExp => 
  ...
  def importTableOps() {
    ...
    compiler (Table) ("groupByReduce", List(A, tpePar("K"), tpePar("V")), MethodSignature(List(Arg("self", Table, None), Arg("keySelector", (A) ==> tpePar("K"), None), Arg("valueSelector", (A) ==> tpePar("V"), None), Arg("reducer", (tpePar("V"),tpePar("V")) ==> tpePar("V"), None), Arg("condition", (A) ==> MBoolean, None)), Table(tpePar("V"))), implicitArgs = List()) implements hashFilterReduce(scala.Tuple3(tpePar("A"), tpePar("K"), tpePar("V")), 0, "condition", "keySelector", "valueSelector", "zeroType[V]", "reducer")
    compiler (Table) ("bulkDivide", List(A), MethodSignature(List(Arg("self", Table, None), Arg("counts", Table(MInt), None), Arg("avgFunc", (A,MInt) ==> A, None)), Table(A)), implicitArgs = List()) implements zip(scala.Tuple3(tpePar("A"), MInt, tpePar("A")), scala.Tuple2(0, 1), "avgFunc")
    ...
    compiler (Table) ("table_apply", List(A), MethodSignature(List(Arg("self", Table, None), Arg("__arg1", MInt, None)), A), implicitArgs = List()) implements composite("array_apply[A](table_raw_data[A](self), __arg1)")
    ...
    compiler (Table) ("table_update", List(A), MethodSignature(List(Arg("self", Table, None), Arg("i", MInt, None), Arg("e", A, None)), MUnit), implicitArgs = List(), effect = write(0)) implements composite("array_update[A](table_raw_data[A](self), i, e)")
  ...
  table_ensureextra[A](self, len);
  val data: this.Rep[ForgeArray[A]] = table_raw_data[A](self);
  array_copy[A](data, pos, data, pos.$plus(len), table_size[A](self).$minus(pos));
  table_set_size[A](self, table_size[A](self).$plus(len))
}""")
  ...
  val data: this.Rep[ForgeArray[A]] = table_raw_data[A](self);
  __ifThenElse(array_length[A](data).$minus(table_size[A](self)).$less(extra), table_realloc[A](self, table_size[A](self).$plus(extra)), unit(()))
}""")
  ...
  array_copy[A](data, unit(0), d, unit(0), table_size[A](self));
  table_set_raw_data[A](self, d.unsafeImmutable)
}""")
  ...
  val src: this.Rep[ForgeArray[A]] = table_raw_data[A](self);
  val dest: this.Rep[ForgeArray[A]] = table_raw_data[A](__arg2);
