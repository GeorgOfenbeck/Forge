...
trait ShallowGenTypeClasses extends ShallowGenOps {
  ...
  def emitTpeClass(grp: Rep[DSLGroup], opsGrp: DSLOps, stream: PrintWriter) {
    ...
    stream.println("object " + opsGrp.name + " extends scala.math.Numeric.ExtraImplicits {")
    ...
    for (o <- opsGrp.ops) {
      ...
      stream.println("    def " + o.name + makeTpeParsWithBounds(o.tpePars.filterNot(p => tpeCls.tpePars.map(_.name).contains(p.name)))
        + makeOpArgsWithType(o, addParen = o.effect != pure) + makeImplicitArgsWithCtxBoundsWithType(o.implicitArgs, o.tpePars, without = tpeCls.tpePars) + ": " + typify(o.retTpe))
    }
    stream.println("  }")
    stream.println()
    ...
    if (tpeCls.tpePars.length == 1) {
      ...
      stream.println("  implicit class " + tpeCls.name + "2" + tpeCls.name + "Ops" + makeTpeParsWithBounds(boundedTpePars) + "(self: " + typify(tpeCls.tpePars.apply(0)) + ") {")
      for (o <- opsGrp.ops) {
      ...
      for (o <- opsGrp.ops) {
        ...
        stream.println("  def " + makeOpMethodName(o) + makeTpeParsWithBounds(boundedTpePars)
          ...
          + ": " + typify(o.retTpe) + " = implicitly["+quote(tpeCls)+"]." + o.name + makeOpArgs(o, addParen = o.effect != pure))
      }
