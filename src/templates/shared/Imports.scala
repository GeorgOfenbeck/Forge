...
trait BaseGenImports extends ForgeCodeGenBase {
  ...
  def emitLMSImports(stream: PrintWriter) {
    stream.println("import scala.lms.common.{Base,BaseExp,EffectExp,BaseFatExp}")
    stream.println("import scala.lms.common.{ScalaGenBase,ScalaGenEffect,ScalaGenFat,CudaGenFat,CGenFat}")
    stream.println("import scala.lms.util._")
    stream.println("import scala.lms.internal._")
  }
