...
trait ImageOps {
  ...
  def importImageOps() {
    ...
    static (GrayscaleImage) ("apply", Nil, DenseMatrix(MDouble) :: GrayscaleImage) implements allocates(GrayscaleImage, {
  ...
  s"""$arg1"""
    ...
    ImageOps {
      ...
      infix ("numRows") (Nil :: MInt) implements redirect {
        val self = quotedArg("self")
        s"""$self.data.numRows"""
      }
      infix ("numCols") (Nil :: MInt) implements redirect {
  val self = quotedArg("self")
  s"""$self.data.numCols"""
}
      ...
      infix ("downsample") (CurriedMethodSignature(List(List(("rowFactor", MInt), ("colFactor", MInt)), List(("sample", GrayscaleImage ==> MDouble))), GrayscaleImage)) implements composite {
          val self = quotedArg("self")
          s"""val sampledData = (0::($self.numRows / rowFactor), 0::($self.numCols / colFactor))  { (i,j) =>  
...
GrayscaleImage(sampledData)"""
      ...
      infix ("convolve") (("kernel", DenseMatrix(MDouble)) :: GrayscaleImage) implements composite {
          val self = quotedArg("self")
          s"""$self.windowedFilter(kernel.numRows, kernel.numCols) { slice => (slice.data *:* kernel).sum }"""
        }
...
GrayscaleImage(out)"""
      ...
      infix ("histogram") (Nil :: DenseVector(MInt)) implements composite {
          val self = quotedArg("self")
          s"""val buckets = $self.data.flattenToVector.groupByReduce(e => e.toInt, e => 1, (a: Rep[Int],b: Rep[Int]) => a+b)
